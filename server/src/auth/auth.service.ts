...
export class AuthService {
 ...
 async login(code: string) {
  ...
  try {
   ...
   await connection.execute('UPDATE el_users SET last_login_at=NOW(3) WHERE id=? AND is_del=0', [userId]);
   const token = randomBytes(32).toString('hex');
   await connection.execute(`INSERT INTO el_sessions(token_hash,user_id,expires_at)
     VALUES(?,?,DATE_ADD(NOW(3),INTERVAL 30 DAY))`, [tokenHash(token), userId]);
 ...
 async logout(identity: Identity) {
  await this.db.pool.execute(`UPDATE el_sessions SET revoked_at=NOW(3)
