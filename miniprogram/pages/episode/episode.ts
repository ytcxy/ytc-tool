...
Page({
 ...
 _visible:false,_loadPromise:null as Promise<void>|null,_retrySilent:false,
 ...
 onHide(){this._visible=false;this._observer?.disconnect();if(this._positionTimer)clearTimeout(this._positionTimer);this._positionTimer=0;this.flushPosition();},
 onUnload(){this._visible=false;this._observer?.disconnect();if(this._positionTimer)clearTimeout(this._positionTimer);this._positionTimer=0;this.flushPosition();},
 load():Promise<void>{
 ...
 enqueue(job:()=>Promise<void>,silent=false){
  ...
  this._queue=this._queue.then(async()=>{
   ...
   catch(e){this._retry=job;this._retrySilent=silent;this.setData({saveError:'未同步：'+errorMessage(e),loggedIn:!!getToken()});}
   finally{if(!silent)this.setData({saving:false});}
  });
 },
 retry(){if(this._retry&&requireLogin())this.enqueue(this._retry,this._retrySilent);},
